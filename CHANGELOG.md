...
- **ストリーム映像へのQRコード・URL上書き表示（QR Code Overlay）**:
  ...
  - **動画再生時のリアルタイム合成**:
    - `overlay_qr_video` が有効な場合、FFmpegの `overlay` フィルタで映像上にリアルタイム合成。
    - 無効時（デフォルト）は従来通り `-c:v copy` の超軽量ストリームコピー（CPU負荷ほぼゼロ）で動作。
  - **写真スライドショー時の自動合成**:
    - `overlay_qr_image` が有効な場合、写真にQR・URLカードを自動合成。
  - **設定ダイアログ (Settings UI) & API 連動**:
    - ⚙ Settings モーダルから「動画オーバーレイ」「写真オーバーレイ」「表示レイアウト（右下/フル画面）」を直感的に切り替え・保存可能。
