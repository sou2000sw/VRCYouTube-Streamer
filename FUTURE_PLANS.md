...
## 9. 📻 再生モード3分岐 ＆ 写真・動画キュー完全分離 ＆ スライドショー安定化 (Playback Modes & Photo Pool Separation) 【要対応 ⚠️】

### 概要
動画・ラジオ・スライドショーの3大再生モードへの明確な分岐、写真プール（アルバム）と動画キューの完全分離、および写真0枚時のフォールバック案内表記（パターンA）を導入し、スライドショー再生の動作安定化とキュー混在による不具合を根本解決する。

### 現状と課題
1. **キュー混在による再生トラブル**:
   - `play_queue` に動画と写真が混在していたため、`[写真, 動画, 写真]` の順でキューに入った場合に動画再生中に後ろの写真が待機状態のまま再生されず、スライドショーが途切れる。
2. **モード定義の曖昧さ**:
   - `radio_mode` (bool) のみで管理されており、「動画」「ラジオ」「写真スライドショー」の独立した切り替えが直感的に行えない。
3. **写真0枚時のハンドリングと案内**:
   - 写真が1枚もない状態でスライドショーが選ばれた場合、単に待機画面が出るだけでは視聴者が状況を判別しにくい。
...
2. **動画キュー (`video_queue`) と 写真プール (`photo_pool`) の完全分離**:
   - アップロードされた写真は写真プールに永続保持され、動画を再生しても消滅しない。いつでもスライドショー素材として利用可能。
3. **写真プール（UI / API）での並び替え・削除機能**:
   - WebリモコンおよびGUI上で、動画キューと同じように写真の「個別削除」「並び替え（上へ/下へ・ドラッグ移動）」「全削除（クリア）」が可能。
4. **写真0枚時のフォールバック案内（パターンA）**:
   - 写真が0枚のときは待機画面に下部案内バー（`📷 スライドショー写真が未登録です（Webリモコンから写真をアップロードできます）`）を合成表示。写真が投稿され次第即座にスライドショーへ復帰。
5. **`photo_pool` の完全クリーンアップ（セッション限定・残骸ゼロ）**:
   - **ソフト終了時および次回起動時** に、`photo_pool` 内の画像および `hls_output/images/` 配下の全キャッシュ画像を完全削除。PC内に一時写真が残留しない安全設計。
6. **スライドショー生成パイプラインの堅牢化**:
   - 写真プール内の全写真を設定された並び順で確実にマニフェスト化し、解像度正規化（1920x1080）を行う。

### 変更対象予定ファイル
- `streamer_core.py`（`playback_mode` 実装、キューと写真プールの分離、写真0枚フォールバック案内バー合成）
- `api_server.py`（`/api/status`, `/api/control`, `/api/upload` の 3モード・写真プール対応）
- `ui/index.html` / `plugin/ui/index.html`（3ボタントグルUI、写真管理UIの整理）
- `gui_streamer.py`（GUIモード切替にスライドショー追加）
- `test_playback_modes.py`（3モード動作検証テスト）
